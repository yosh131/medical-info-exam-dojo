...
import { Brain, CheckCircle2, ChevronLeft, ChevronRight, Clipboard, Plus, Shuffle, X, XCircle } from "lucide-react";
...
import { CARD_TYPE_LABELS, CONFIDENCE_LABELS, ERROR_LABELS, SUBJECT_LABELS, type CardType, type Confidence, type ErrorReason, type Question, type ReadingMistakeType, type Subject } from "@/lib/types";
...
function PracticeSetup({questions}:{questions:Question[]|undefined}) {
  ...
  const [selectedYears,setSelectedYears]=useState<number[]>([]); const [selectedSubjects,setSelectedSubjects]=useState<Subject[]>([]); const initialized=useRef(false);
  useEffect(()=>{if(!initialized.current&&questions?.length){setSelectedYears(years);setSelectedSubjects(subjects);initialized.current=true}},[questions,subjects,years]);
  const matching=useMemo(()=>filterPracticeQuestions(questions??[],new Set(selectedYears),new Set(selectedSubjects)),[questions,selectedSubjects,selectedYears]);
  ...
  function start(){
    ...
    const session=shufflePracticeQuestions(matching,seed); const next=new URLSearchParams();
    next.set("id",session[0].id); next.set("years",selectedYears.slice().sort((a,b)=>b-a).join(",")); next.set("subjects",subjectOrder.filter((subject)=>selectedSubjects.includes(subject)).join(",")); next.set("seed",seed);
    router.push(`/practice?${next.toString()}`);
  }
  if(questions===undefined)return <><PageHeader eyebrow="Practice" title="出題設定"/><div className="card"><p>問題を読み込んでいます。</p></div></>;
  ...
  return <><PageHeader eyebrow="Practice" title="出題設定"/><section className="practice-setup">
    ...
    <div className="setup-summary"><div><b>{matching.length}問</b><span>選択範囲からランダム出題</span></div><button className="button" disabled={!matching.length} onClick={start}><Shuffle size={18}/>演習を開始</button></div>
  </section></>;
}

function Practice() {
  const params=useSearchParams(), router=useRouter(); const requested=params.get("id");
  const allQuery=useLiveQuery(async()=>sortPracticeQuestions(await db.questions.toArray()),[]); const all=allQuery??[];
  ...
  const sessionQuestions=useMemo(()=>{
    const sessionParams=new URLSearchParams(sessionKey); const seed=sessionParams.get("seed"), yearsValue=sessionParams.get("years"), subjectsValue=sessionParams.get("subjects");
    ...
    return shufflePracticeQuestions(filterPracticeQuestions(all,years,subjects),seed);
  },[all,sessionKey]);
  ...
  const [answers,setAnswers]=useState<string[]>([]); const [confidence,setConfidence]=useState<Confidence>("medium"); const [attemptId,setAttemptId]=useState<string>(); const [correct,setCorrect]=useState<boolean>(); const [grading,setGrading]=useState(false); const started=useRef(Date.now()); const resultRef=useRef<HTMLElement>(null);
  ...
  useEffect(()=>{setAnswers([]);setConfidence("medium");setAttemptId(undefined);setCorrect(undefined);setGrading(false);setPrimary(undefined);setSecondary([]);setReading(undefined);setNote("");setAnalysisSaved(false);setCardOpen(false);setMessage("");started.current=Date.now()},[question?.id]);
  ...
  async function toggle(label:string){if(correct!==undefined||grading)return;if(multiple){setAnswers((old)=>old.includes(label)?old.filter((x)=>x!==label):[...old,label]);return}const selected=[label];setAnswers(selected);await grade(selected)}
  async function grade(selected=answers){if(!question||!selected.length||grading||correct!==undefined)return;setGrading(true);try{const isCorrect=isCorrectAnswer(selected,question.correctAnswer);const id=makeId();await db.attempts.add({id,questionId:question.id,userAnswer:multiple?selected:selected[0],isCorrect,confidence,elapsedSec:Math.round((Date.now()-started.current)/1000),attemptedAt:isoNow()});setAttemptId(id);setCorrect(isCorrect);if(!isCorrect||confidence==="low")await ensureSchedule("question",question.id,1)}finally{setGrading(false)}}
  ...
  if(!requested)return <PracticeSetup questions={allQuery}/>;
  ...
  return <><PageHeader eyebrow={`${question.examYear} / ${SUBJECT_LABELS[question.subject]} · ${currentIndex+1} / ${sessionQuestions.length}`} title={`問 ${question.questionNo}`}/>
    ...
    {correct===undefined&&multiple&&<button className="button primary-action" disabled={!answers.length||grading} onClick={()=>void grade()}>{grading?"採点中…":"回答を確定"}</button>}
    {correct!==undefined && <section ref={resultRef} role="status" aria-live="polite" className={`result-panel result-reveal ${correct?"is-correct":"is-wrong"}`}><div className="result-verdict">{correct?<CheckCircle2 aria-hidden="true"/>:<XCircle aria-hidden="true"/>}<strong>{correct?"正解です":"不正解です"}</strong></div><div><b>正解：</b>{Array.isArray(question.correctAnswer)?question.correctAnswer.join(", "):question.correctAnswer}</div>{displayedExplanation&&<div><b>この問題の解説</b><p className="explanation-text">{displayedExplanation}</p></div>}<SafeTable html={question.explanationTableHtml}/><QuestionMediaList items={media.filter((item)=>item.role==="explanation")}/></section>}
