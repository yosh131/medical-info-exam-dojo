...
function Practice() {
  ...
  return <><PageHeader eyebrow={`${question.examYear} / ${SUBJECT_LABELS[question.subject]} · ${currentIndex+1} / ${sessionQuestions.length}`} title={`問 ${question.questionNo}`}/>
    ...
    {correct!==undefined && <section ref={resultRef} role="status" aria-live="polite" className={`result-panel result-reveal ${correct?"is-correct":"is-wrong"}`}><div className="result-verdict">{correct?<CheckCircle2 aria-hidden="true"/>:<XCircle aria-hidden="true"/>}<strong>{correct?"正解です":"不正解です"}</strong></div><div><b>正解：</b>{Array.isArray(question.correctAnswer)?question.correctAnswer.join(", "):question.correctAnswer}</div>{question.explanation&&<div><b>この問題の解説</b><p style={{whiteSpace:"pre-wrap",lineHeight:1.7}}>{question.explanation}</p></div>}<SafeTable html={question.explanationTableHtml}/><QuestionMediaList items={media.filter((item)=>item.role==="explanation")}/></section>}
