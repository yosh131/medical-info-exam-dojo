...
function Practice() {
  ...
  function toggle(label:string){if(correct!==undefined)return;setAnswers((old)=>multiple?(old.includes(label)?old.filter((x)=>x!==label):[...old,label]):[label])}
  async function grade(){if(!question||!confidence||!answers.length)return;const isCorrect=isCorrectAnswer(answers,question.correctAnswer);const id=makeId();await db.attempts.add({id,questionId:question.id,userAnswer:multiple?answers:answers[0],isCorrect,confidence,elapsedSec:Math.round((Date.now()-started.current)/1000),attemptedAt:isoNow()});setAttemptId(id);setCorrect(isCorrect);if(!isCorrect||confidence==="low")await ensureSchedule("question",question.id,1)}
  ...
  function next(){if(!question)return;const i=all.findIndex((q)=>q.id===question.id);const target=all[(i+1)%all.length];if(target)router.push(`/practice?id=${target.id}`)}
  ...
  return <><PageHeader eyebrow={`${question.examYear} / ${SUBJECT_LABELS[question.subject]}`} title={`問 ${question.questionNo}`}/>
    ...
    {message&&<p className="success tiny">{message}</p>}{attemptId&&<button className="button" style={{width:"100%",marginTop:12}} onClick={next}>次の問題 <ArrowRight size={18}/></button>}
