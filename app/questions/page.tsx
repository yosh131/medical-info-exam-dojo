...
import { isErrorReason } from "@/lib/reasonFilters";
import { ERROR_LABELS, SUBJECT_LABELS, type ErrorReason, type Subject } from "@/lib/types";

function QuestionList() {
  const params=useSearchParams(); const reasonParam=params.get("reason"); const [subject,setSubject]=useState<Subject|"">(""); const [year,setYear]=useState(""); const [reason,setReason]=useState<ErrorReason|"">(isErrorReason(reasonParam)?reasonParam:""); const [state,setState]=useState(params.get("filter")??""); const [search,setSearch]=useState("");
  ...
  return <><PageHeader eyebrow="Question bank" title="問題一覧"/>
    ...
    {reason&&<div className="inline-status" style={{marginBottom:12}}>A〜F分類 <b>{reason}</b> で絞り込み中。<Link href={`/practice?reason=${reason}`} style={{marginLeft:8}}>この分類だけ出題</Link></div>}
