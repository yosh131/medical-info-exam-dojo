...
function QuestionList() {
  ...
  const rows=useMemo(()=>data.questions.filter((q)=>{
    ...
    if(state==="low"&&!attempts.some((a)=>a.confidence==="low"))return false; if(state==="repeat"&&attempts.filter((a)=>!a.isCorrect).length<2)return false;
    ...
    return true;
  }),[data,subject,year,reason,state,search]);
