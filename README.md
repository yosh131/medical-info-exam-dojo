...
非公開の問題バンドルから、メディア付き問題を優先した少量データを生成できます。
...
開発モードの「問題を取り込む」にだけ「デバッグデータを読み込む」が表示されます。生成先の `public/__debug__/` はGit管理対象外です。また、本番ビルド時にこのディレクトリやZIP・DB・画像・PDFが `public/` に存在すると安全検査でビルドが停止します。

本番ビルド前にローカルデバッグデータを削除する場合は次を実行します。
